...
_STEP_USER_SCHEMA = vol.Schema({ vol.Optional(CONF_OMDB_API_KEY): str })
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        schema = vol.Schema({
            vol.Optional(
                CONF_OMDB_API_KEY,
                default=self._entry.options.get(CONF_OMDB_API_KEY, self._entry.data.get(CONF_OMDB_API_KEY, ""))
            ): str
        })
